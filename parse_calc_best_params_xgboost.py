...
def open_scores(file_name):
    ...
    """
    cols = ["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method", "score_train", "score_test"]
    hyper_param_list = ["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method"]
    ...
    runlog[['cv', 'eta']] = runlog['eta'].str.split('=', 1, expand=True)
    ...
    runlog["mean_score_train"] = runlog.groupby(["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method"])["score_train"].transform("mean")
    runlog["mean_score_test"] = runlog.groupby(["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method"])["score_test"].transform("mean")
    runlog["mean_run_time"] = runlog.groupby(["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method"])["run_time"].transform("mean")
    # Check how many rows for each model and remove the ones with less than 5 rows
    runlog["count_score_test"] = runlog.groupby(["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method"])["score_test"].transform("count")
...
def analyze_scores_choose_params(scores_file):
    """
    ...
    """    
    print(scores_file)
    score_tab = scores_file.groupby(["eta", "max_depth", "min_child_weight", "n_estimators", "objective",
                                     "tree_method"])["mean_score_test"].mean().reset_index().\
...
if __name__ == "__main__":
    ...
    working_dir = "/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/log_results_first_tuning"
    # find all the log files in the working directory that have k5 in the name and end with .log
    log_files = [f for f in os.listdir(working_dir) if re.search(r'iter2.*.log', f) and f.endswith('.log')]
    ...
    # log_files = [f for f in os.listdir(working_dir) if f.endswith('.log')]
    # create a dataframe for the scores
    scores_file = pd.DataFrame(columns=["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method", "score_train", "score_test"])
    # loop over all the log files
    for log_file in log_files:
        print(log_file)
        # open the log file
        runlog = open_scores(os.path.join(working_dir, log_file))
    ...
    scores_file.to_pickle(os.path.join(working_dir, "scores_iter2.pkl"))
    print(scores_file)
    print(scores_file.shape)
    best_params = analyze_scores_choose_params(scores_file)
