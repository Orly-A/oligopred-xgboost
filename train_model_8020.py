...
def data_definition():
    ...
    pickle.dump(X_train, open("/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/X_train_80.pkl", "wb"))
    pickle.dump(y_train, open("/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/y_train_80.pkl", "wb"))
    pickle.dump(X_test, open("/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/X_test_20.pkl", "wb"))
    pickle.dump(y_test, open("/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/y_test_20.pkl", "wb"))
...
def initial_xgboost_classifier(X_train,y_train, X_test=None, y_test=None):
    xgb_class = xgb.XGBClassifier(objective='multi:softprob', eta=0.3, max_depth=6, min_child_weight=5,
                                  n_estimators=800, tree_method="exact", random_state=1)
    print("selected this set of parameters from the first hyp_param tuning, and took the best option with 800 trees of less."
          "'objective='multi:softprob', eta=0.3, max_depth=6, min_child_weight=5, n_estimators=800, tree_method='exact', random_state=1'")
    ...
    pickle.dump(xgb_class, open("/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/xgb_8020_800_est.pkl", "wb"))
    joblib.dump(xgb_class, "/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/xgb_8020_800_est.joblib")
