...
def open_scores(file_name):
    ...
    """
    cols = ["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method", "score_train", "score_test"]
    hyper_param_list = ["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth",
                        "min_child_weight", "n_estimators", "objective", "tree_method"]
    ...
    runlog["mean_score_train"] = \
    runlog.groupby(["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method"])[
        "score_train"].transform("mean")
    runlog["mean_score_test"] = \
    runlog.groupby(["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method"])[
        "score_test"].transform("mean")
    runlog["mean_run_time"] = \
    runlog.groupby(["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method"])[
        "run_time"].transform("mean")
    # Check how many rows for each model and remove the ones with less than 5 rows
    runlog["count_score_test"] = \
    runlog.groupby(["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method"])[
        "score_test"].transform("count")
...
def analyze_scores_choose_params(scores_file):
    ...
    """
    score_tab = scores_file.groupby(["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth", "min_child_weight",
            "n_estimators", "objective", "tree_method"])["mean_score_test"].mean().reset_index(). \
...
if __name__ == "__main__":
    ...
    working_dir = "/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/log_results_third_tuning"
    # find all the log files in the working directory that have k5 in the name and end with .log
    log_files = [f for f in os.listdir(working_dir) if re.search(r'colsample.*.log', f) and f.endswith('.log')]
    ...
    scores_file = pd.DataFrame(
        columns=["eta", "max_depth", "min_child_weight", "n_estimators", "objective", "tree_method", "score_train",
                 "score_test"])
    ...
    scores_file.to_pickle(os.path.join(working_dir, "scores_colsample.pkl"))
    print(scores_file)
