...
def open_scores(file_name, hyper_param_list):
    ...
    """
    cols = ["colsample_bylevel", "colsample_bynode", "colsample_bytree", "eta", "max_depth",
            "min_child_weight", "n_estimators", "objective", "reg_lambda", "tree_method",
...
if __name__ == "__main__":
    ...
    working_dir = "/vol/ek/Home/orlyl02/working_dir/oligopred/xgboost/log_results_fifth_tuning"
    # find all the log files in the working directory that have k5 in the name and end with .log
    log_files = [f for f in os.listdir(working_dir) if re.search(r'downsample.*.log', f) and f.endswith('.log')]
    ...
    scores_file.to_pickle(os.path.join(working_dir, "scores_downsample_first.pkl"))
    print(scores_file)
